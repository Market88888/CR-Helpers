...
--  Сравнение с GitHub: version.txt в репо Market88888/CR-Helpers
...
local PCS_UPDATE_AUTO_SECONDS = 300  -- проверка + уведомление каждые 5 минут
...
--
-- Всё завёрнуто в ОДНУ глобальную таблицу PCS_UPDATE (а не в
-- десяток отдельных local'ов) по двум причинам:
--   1) файл и так близок к лимиту Lua/LuaJIT в 200 локальных
--      переменных на функцию (весь файл — это одна "функция чанка"),
--      а глобальная переменная слот локальной переменной не занимает;
--   2) поле такой таблицы (PCS_UPDATE.state, PCS_UPDATE.check и т.д.)
--      можно читать из ЛЮБОГО места файла — в т.ч. из вкладки
--      "О скрипте" (см. drawAboutInner), которая объявлена намного
--      ниже по файлу и не видела бы обычные local-переменные отсюда.
--
...
-- То же самое доступно кнопками во вкладке "О скрипте" → "Обновления".
-- ============================================================
PCS_UPDATE = (function()
    local M = {}
    ...
    M.cfg = {
        ...
        -- имя файла скрипта в репозитории (main)
        file         = 'PCStats.lua',
        version_file = 'version.txt',
    }
    M.cfg.autoCheck = true
    M.cfg.api_release = ('https://api.github.com/repos/%s/%s/releases/latest'):format(M.cfg.github_user, M.cfg.github_repo)
    M.cfg.api_tags    = ('https://api.github.com/repos/%s/%s/tags'):format(M.cfg.github_user, M.cfg.github_repo)
    M.cfg.raw_ver     = ('https://raw.githubusercontent.com/%s/%s/main/%s'):format(M.cfg.github_user, M.cfg.github_repo, M.cfg.version_file)
    -- список URL для скачивания обновления (по порядку)
    M.cfg.dl_urls = {
        -- актуальный файл скрипта в репо (имя = M.cfg.file)
        ('https://raw.githubusercontent.com/%s/%s/main/%s'):format(M.cfg.github_user, M.cfg.github_repo, M.cfg.file),
        ('https://cdn.jsdelivr.net/gh/%s/%s@main/%s'):format(M.cfg.github_user, M.cfg.github_repo, M.cfg.file),
        -- cache-bust через query (часть CDN режет, raw.githubusercontent обычно ок)
        ('https://raw.githubusercontent.com/%s/%s/main/%s?raw=1'):format(M.cfg.github_user, M.cfg.github_repo, M.cfg.file),
    }
    M.cfg.raw_main   = M.cfg.dl_urls[1]
    M.cfg.raw_master = M.cfg.dl_urls[1]
    M.cfg.release_dl = M.cfg.dl_urls[1]


    M.effil = safeRequire('effil')
    ...
    M.state = {
        ...
        last_check = 0,
        last_error = nil,
        dlProg     = 0,   -- 0..100 прогресс скачивания
        dlStatus   = "",  -- текст статуса (скачивание / запись / готово)
    }

    local function normVer(v)
        if not v or v == '' then return nil end
        return tostring(v):gsub('^[vV]', '')
    end

    local function verLt(a, b)
    ...
    local function verFromBody(body)
        if not body or body == '' then return nil end
        return normVer(body:match('script_version%s*%(%s*["\']([^"\']+)["\']')
            or body:match('SCRIPT_VER%s*=%s*["\']([^"\']+)["\']'))
    end

    local function parseRelease(body)
        return normVer(body and body:match('"tag_name"%s*:%s*"([^"]+)"'))
    end

    local function parseFirstTag(body)
        return normVer(body and body:match('"name"%s*:%s*"([^"]+)"'))
    end

    local function isValidBody(body)
        if type(body) ~= 'string' or #body < 500 then return false end
        local headPart = body:sub(1, 500)
        local low = headPart:lower()
        if low:find('<!doctype html', 1, true) or low:find('<html', 1, true) then return false end
        if body:find('404: Not Found', 1, true) and #body < 800 then return false end
        if body:find('Not Found', 1, true) and #body < 400 then return false end
        if body:find('script_name%s*%(') then return true end
        if body:find('SCRIPT_VER%s*=') then return true end
        if body:find('script_version%s*%(') then return true end
        return #body > 50000  -- большой lua-файл без явных маркеров всё равно принимаем
    end
    ...
    local function writeAtomic(path, content)
        ...
        os.remove(path)
        local renamed = false
        pcall(function() renamed = os.rename(tmp, path) end)
    ...
    -- HTTP GET без обязательного effil: requests / downloadUrlToFile / urlmon + зеркала
    local function nocache(u)
        local sep = (tostring(u):find("?", 1, true) and "&") or "?"
        return u .. sep .. "t=" .. tostring(os.time()) .. tostring(math.random(100, 999))
    end

    local function httpGetBody(url)
        url = tostring(url or "")
        if url == "" then return nil, "empty url" end
        local urls = { nocache(url) }
        -- jsDelivr mirror for raw.githubusercontent.com
        local u, r, b, f = url:match("https://raw%.githubusercontent%.com/([^/]+)/([^/]+)/([^/]+)/(.+)")
        if u then
            urls[#urls + 1] = nocache(string.format(
                "https://cdn.jsdelivr.net/gh/%s/%s@%s/%s", u, r, b, f))
        end

        for _, tryUrl in ipairs(urls) do
            -- 1) requests
            local okR, requests = pcall(require, "requests")
            if okR and requests then
                local ok, response
                if requests.get then
                    ok, response = pcall(requests.get, tryUrl, {
                        headers = { ["User-Agent"] = "PCStats-AutoUpdate/1.0", ["Cache-Control"] = "no-cache" },
                        timeout = 15,
                    })
                elseif requests.request then
                    ok, response = pcall(requests.request, "GET", tryUrl, {
                        headers = { ["User-Agent"] = "PCStats-AutoUpdate/1.0" },
                        timeout = 15,
                    })
                end
                if ok and response then
                    local code = tonumber(response.status_code or response.status) or 0
                    local body = response.text or response.content or ""
                    if code == 200 and type(body) == "string" and #body > 0
            ...
            if type(downloadUrlToFile) == "function" then
                local tmp = "moonloader/PCStats_http.tmp"
                pcall(function()
                    if getWorkingDirectory then
                        tmp = getWorkingDirectory() .. "/PCStats_http.tmp"
                    end
                end)
                pcall(os.remove, tmp)
                local done, okDl = false, false
                pcall(downloadUrlToFile, tryUrl, tmp, function(id, status, p1, p2)
                    if status == 58 then -- download finished status in some builds
                        done = true; okDl = true
                    elseif status == 6 or status == 7 then -- errors
                        done = true; okDl = false
                    end
                end)
                local waited = 0
                while waited < 12000 do
                    ...
                    if f then
                        ...
                        if body and #body > 2 and not body:find("404: Not Found", 1, true)
                            and not body:lower():find("<!doctype html", 1, true) then
                            pcall(os.remove, tmp)
                            return body, nil
                        end
                    end
            ...
            if ffi then
                pcall(function()
                    ffi.cdef[[ int URLDownloadToFileA(void* p, const char* u, const char* f, unsigned long, void*); ]]
                end)
                local urlmon
                pcall(function() urlmon = ffi.load("urlmon") end)
                if urlmon and urlmon.URLDownloadToFileA then
                    local tmp = "moonloader/PCStats_http.tmp"
                    pcall(function()
                        if getWorkingDirectory then tmp = getWorkingDirectory() .. "/PCStats_http.tmp" end
                    end)
                    pcall(os.remove, tmp)
                    local hr = urlmon.URLDownloadToFileA(nil, tryUrl, tmp, 0, nil)
                    if hr == 0 then
                        local f = io.open(tmp, "rb")
                        if f then
                            local body = f:read("*a"); f:close()
                            pcall(os.remove, tmp)
                            if body and #body > 2 and not body:find("404: Not Found", 1, true) then
                                return body, nil
                            end
                        end
                    end
        ...
        return nil, "no downloader"
    end

    local function httpFetch(url)

        if not M.effil then return nil end
        return M.effil.thread(function(target)
            local req = require('requests')
            local ok, response = pcall(req.request, 'GET', target, {
                headers = { ['User-Agent'] = 'PCStats-AutoUpdate/1.0' },
                timeout = 20,
            })
            ...
            return true, { status_code = tonumber(response.status_code) or 0, text = response.text or '', error = nil }
        end)(url)
    end

    -- Идёт через уже переопределённый ниже sampAddChatMessage (см.
    -- секцию "ФИКС КАРАКУЛЬ") — сообщение сразу дублируется тостом.
    -- Определяется через pcall(sampAddChatMessage, ...), поэтому
    -- безопасно вызывать даже до того, как SAMP доступен.
    ...
    function M.check(silent)
        if M.state.checking or M.state.installing then return end
        M.state.checking = true
        M.state.last_error = nil
        if not silent then
            notify("\xcf\xf0\xee\xe2\xe5\xf0\xea\xe0\x20\xee\xe1\xed\xee\xe2\xeb\xe5\xed\xe8\xe9\x2e\x2e\x2e")
        end

        lua_thread.create(function()
            local remote_ver = nil
            -- 1) ТОЛЬКО version.txt — единственный источник правды для номера версии
            local verUrls = {
                M.cfg.raw_ver,
                ('https://cdn.jsdelivr.net/gh/%s/%s@main/%s'):format(M.cfg.github_user, M.cfg.github_repo, M.cfg.version_file),
            }
            for _, url in ipairs(verUrls) do
                local body, err = httpGetBody(url)
                if body and #body > 0 and #body < 200 then
                    local line = body:match("([^\r\n]+)") or body
                    line = line:gsub("^\239\187\191", ""):gsub("^%s+", ""):gsub("%s+$", ""):gsub("^[vV]", "")
                    local pure = line:match("^(%d+%.%d+[%d%.]*)")
                    if pure and pure ~= "404" then
                        remote_ver = normVer(pure)
                        break
                    end
                end
            end
            -- 2) запасной: SCRIPT_VER из PCStats.lua на GitHub (если version.txt недоступен)
            if not remote_ver then
                local fb = { M.cfg.raw_main }
                if M.cfg.dl_urls then
                    for _, u in ipairs(M.cfg.dl_urls) do fb[#fb + 1] = u end
                end
                for _, url in ipairs(fb) do
                    if url then
                        local body = select(1, httpGetBody(url))
                        if body and #body > 500 then
                            remote_ver = verFromBody(body)
                            if remote_ver then break end
                        end
                    end
                end
            end

            -- 2) GitHub API через effil (если есть)
            if not remote_ver and M.effil then
                for _, item in ipairs({
                    { url = M.cfg.api_release, mode = 'release' },
                    { url = M.cfg.api_tags, mode = 'tags' },
                }) do
                    local okT, thr = pcall(httpFetch, item.url)
                    if okT and thr then
                        for _ = 1, 100 do
                            wait(100)
                            local st, er = thr:status()
                            if st == 'completed' then
                                local ok, response = thr:get()
                                if ok and response and tonumber(response.status_code) == 200 then
                                    if item.mode == 'release' then
                                        remote_ver = parseRelease(response.text)
                                    else
                                        remote_ver = parseFirstTag(response.text)
                                    end
                                end
                                break
                            end
                            if not st or st == 'failed' or st == 'canceled' or er then break end
                        end
                    end
                    if remote_ver then break end
                end
            ...
            if remote_ver then
                M.state.ver_remote = remote_ver
                M.state.available = verLt(SCRIPT_VER, remote_ver)
                M.state.last_error = nil
                if M.state.available then
                    -- уведомление: сразу при нахождении + каждые 5 минут, пока не обновят
                    local lastN = tonumber(M.state.last_notify_ver_time) or 0
                    local needNotify = (not silent)
                        or (M.state.last_notify_ver ~= remote_ver)
                        or ((os.time() - lastN) >= 300)
                    if needNotify then
                        notify("äîñòóïíî îáíîâëåíèå v" .. remote_ver, "{FFD700}")
                        M.state.last_notify_ver = remote_ver
                        M.state.last_notify_ver_time = os.time()
                    end
                else
                    -- актуально: сбрасываем флаги уведомлений
                    M.state.last_notify_ver = nil
                    M.state.last_notify_ver_time = 0
                    if not silent then
                        notify("óñòàíîâëåíà àêòóàëüíàÿ âåðñèÿ v" .. SCRIPT_VER, "{00FF88}")
                    end
                end
            else
                M.state.last_error = 'check failed'
                if not silent then
                    notify("\xcd\xe5\x20\xf3\xe4\xe0\xeb\xee\xf1\xfc\x20\xef\xf0\xee\xe2\xe5\xf0\xe8\xf2\xfc\x20\xe2\xe5\xf0\xf1\xe8\xfe\x20\xed\xe0\x20\x47\x69\x74\x48\x75\x62", "{FF6666}")
                end
            end
        end)
    end

    -- Скачивает актуальный файл с GitHub и перезаписывает им СЕБЯ
    -- САМОГО на диске (тот же путь, что вернёт thisScript().path),
    -- затем пробует горячую перезагрузку через штатный script:reload()
    -- — если в этой сборке MoonLoader его нет, откатывается на
    -- unload() с сообщением в чат, и игроку нужно запустить скрипт
    -- заново вручную (папка moonloader / F4)
    function M.install()
        if M.state.installing then
            notify("\xd3\xe6\xe5\x20\xe8\xe4\xb8\xf2\x20\xf3\xf1\xf2\xe0\xed\xee\xe2\xea\xe0\x20\xee\xe1\xed\xee\xe2\xeb\xe5\xed\xe8\xff", "{FFAA00}")
            return
        end
        local selfPath = nil
        pcall(function()
            if thisScript and thisScript() then selfPath = thisScript().path end
        end)
        if not selfPath or selfPath == "" then
            notify("\xcd\xe5\x20\xf3\xe4\xe0\xeb\xee\xf1\xfc\x20\xee\xef\xf0\xe5\xe4\xe5\xeb\xe8\xf2\xfc\x20\xef\xf3\xf2\xfc\x20\xea\x20\xf1\xe2\xee\xe5\xec\xf3\x20\xf4\xe0\xe9\xeb\xf3", "{FF6666}")
            return
        end
        M.state.installing = true
        M.state.dlProg = 0
        M.state.dlProgShow = 0
        M.state.dlStatus = "..."
        notify("\xcd\xe0\xf7\xe8\xed\xe0\xfe\x20\xf1\xea\xe0\xf7\xe8\xe2\xe0\xed\xe8\xe5\x20\xee\xe1\xed\xee\xe2\xeb\xe5\xed\xe8\xff\x2e\x2e\x2e")

        lua_thread.create(function()
            local function setProg(p, status)
                p = tonumber(p) or 0
                if p < 0 then p = 0 end
                if p > 100 then p = 100 end
                M.state.dlProg = p
                if status then M.state.dlStatus = status end
            end

            local urls = {}
            if M.cfg.dl_urls then
                for _, u in ipairs(M.cfg.dl_urls) do urls[#urls + 1] = u end
            end
            if M.cfg.raw_main then urls[#urls + 1] = M.cfg.raw_main end
            -- убрать дубли
            local seen, uniq = {}, {}
            ...
            local body = nil
            local nUrl = math.max(#urls, 1)
            for i, url in ipairs(urls) do
                local target = math.floor(5 + (i - 1) * (65 / nUrl))
                -- плавно дотягиваем полоску до target
                local cur = tonumber(M.state.dlProg) or 0
                while cur < target do
                ...
                setProg(target, nil)
                local b, err = httpGetBody(url)
                if b and isValidBody(b) then
                    body = b
                    -- плавно к 78%
                    while cur < 78 do
                        cur = math.min(78, cur + 3)
                        setProg(cur, nil)
                        wait(25)
                    end
                    setProg(78, nil)
            ...
            if not body then
                ...
                setProg(0, "")
                M.state.last_error = "download failed"
                notify("\xcd\xe5\x20\xf3\xe4\xe0\xeb\xee\xf1\xfc\x20\xf1\xea\xe0\xf7\xe0\xf2\xfc\x20\xee\xe1\xed\xee\xe2\xeb\xe5\xed\xe8\xe5", "{FF6666}")
                return
            end

            local newVer = verFromBody(body)
            setProg(85, nil)
            wait(40)
            pcall(function()
                local old = io.open(selfPath, "rb")
                if old then
                    local data = old:read("*a"); old:close()
                    local bak = io.open(selfPath .. ".bak", "wb")
                    if bak then bak:write(data); bak:close() end
                end
            end)
            setProg(92, nil)
            wait(40)
            if not writeAtomic(selfPath, body) then
                M.state.installing = false
                setProg(0, "")
                notify("\xcd\xe5\x20\xf3\xe4\xe0\xeb\xee\xf1\xfc\x20\xe7\xe0\xef\xe8\xf1\xe0\xf2\xfc\x20\xf4\xe0\xe9\xeb\x20\xf1\xea\xf0\xe8\xef\xf2\xe0", "{FF6666}")
                return
            end
            local cur = 92
            while cur < 100 do
            ...
            M.state.last_notify_ver = nil
            M.state.last_notify_ver_time = 0
            M.state.ver_remote = newVer or M.state.ver_remote
            notify("\xce\xe1\xed\xee\xe2\xeb\xe5\xed\xee\x20\xe4\xee\x20v" .. tostring(newVer or "?") ..
            ...
            if ok and scr then
                if scr.reload then
                    ...
                    notify("\xcf\xe5\xf0\xe5\xe7\xe0\xef\xf3\xf1\xf2\xe8\xf2\xe5\x20\xf1\xea\xf0\xe8\xef\xf2\x20\xe2\xf0\xf3\xf7\xed\xf3\xfe\x20\x28F10\x20/\x20moonloader\x29", "{FFAA00}")
                end
            end
        end)
    end
...
-- Тонкие обёртки-глобалы для команды чата /pcsupdate (см. main())
function pcsCheckForUpdate(silent) PCS_UPDATE.check(silent) end
function pcsInstallUpdate() PCS_UPDATE.install() end
        ...
        aboutCard("##updcard", 130, function(aw, ch)
            ...
            elseif U.state.ver_remote then
                if U.state.available then
                    ...
                    imgui.TextColored(iv4(0.40,1.00,0.50,1.0), "v" .. U.state.ver_remote)
                end
            else
                imgui.TextColored(iv4(0.6,0.6,0.6,1.0), u8"\xed\xe5\xe8\xe7\xe2\xe5\xf1\xf2\xed\xee")
            end
            ...
            if imgui.Button(u8"\xcf\xf0\xee\xe2\xe5\xf0\xe8\xf2\xfc##updcheck", imgui.ImVec2(btnW, btnH)) then
                if U then U.check(false) end
            end
            ...
            -- прогресс скачивания 0..100% (плавная полоска)
            if U and (U.state.installing or (U.state.dlProg and U.state.dlProg > 0)) then
                ...
                -- плавное дотягивание отображаемого %
                if U.state.dlProgShow < target then
                    U.state.dlProgShow = U.state.dlProgShow + math.max(0.4, (target - U.state.dlProgShow) * 0.12)
                    if U.state.dlProgShow > target then U.state.dlProgShow = target end
                elseif U.state.dlProgShow > target then
                    U.state.dlProgShow = target
                end
                local p = U.state.dlProgShow
                local frac = p / 100
                -- цвет: зелёный → чуть ярче к концу
                local g = 0.55 + 0.35 * frac
                imgui.SetCursorPos(imgui.ImVec2(SFtext(16), SFtext(88)))
                imgui.PushStyleColor(imgui.Col.PlotHistogram, iv4(0.15, g, 0.40, 1.0))
                imgui.PushStyleColor(imgui.Col.FrameBg, iv4(0.12, 0.14, 0.18, 1.0))
                imgui.ProgressBar(frac, imgui.ImVec2(aw - SFtext(32), SFtext(22)),
...
function main()
    ...
    if PCS_UPDATE.cfg.autoCheck then
        lua_thread.create(function()
            wait(3000)
            pcall(pcsCheckForUpdate, true)
            local sec = tonumber(PCS_UPDATE_AUTO_SECONDS) or 300
            if sec < 60 then sec = 60 end
            if sec > 900 then sec = 900 end
            while true do
                wait(sec * 1000)
                if PCS_UPDATE and PCS_UPDATE.cfg and PCS_UPDATE.cfg.autoCheck then
                    pcall(pcsCheckForUpdate, true)
                end
            end
        end)

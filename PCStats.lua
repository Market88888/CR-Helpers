...
function pcs_ver.cleanup()
    ...
    pcall(function()
        local sp = pcs_ver.selfPath()
        if sp then pcall(os.remove, sp .. ".new") end
    end)
end
...
function pcs_ver.install()
    ...
    lua_thread.create(function()
        ...
        local okAll, errAll = pcall(function()
            ...
            -- 4) установка: бэкап .bak + атомарная замена
            ...
            if not okRep then
                if pcs_ver.fileSize(selfPath) < 500 then
                    local bak = pcs_ver.readAll(selfPath .. ".bak")
                        or pcs_ver.readAll(selfPath .. ".old")

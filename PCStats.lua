...
local PCS_UPDATE_AUTO_SECONDS = 60  -- проверка версии каждые 60 секунд
...
PCS_UPDATE = (function()
    ...
    M.cfg.dl_urls = {
        ...
        ('https://raw.githubusercontent.com/%s/%s/main/PCStats.lua'):format(M.cfg.github_user, M.cfg.github_repo),
        ('https://cdn.jsdelivr.net/gh/%s/%s@main/PCStats.lua'):format(M.cfg.github_user, M.cfg.github_repo),
    }
    ...
    function M.check(silent)
        ...
        lua_thread.create(function()
            ...
            -- 1) version.txt (самый быстрый и надёжный)
            local sources = {
                M.cfg.raw_ver,
                M.cfg.raw_main,
                M.cfg.raw_master,
            ...
            for _, url in ipairs(sources) do
                ...
                if body and #body > 0 then
                    if url == M.cfg.raw_ver or url:find("version%.txt") then
                        local line = body:match("([^\r\n]+)") or body
                        line = line:gsub("^%s+", ""):gsub("%s+$", ""):gsub("^[vV]", "")
                        if line:match("^%d+%.%d+") then
                            remote_ver = normVer(line)
                        end
                    else
            ...
            if remote_ver then
                ...
                if M.state.available then
                    ...
                    local same = (M.state.last_notify_ver == remote_ver)
                    if (not silent) or (not same) or (os.time() - lastN > 3600) then
                        notify("\xe4\xee\xf1\xf2\xf3\xef\xed\xee\x20\xee\xe1\xed\xee\xe2\xeb\xe5\xed\xe8\xe5\x20v" .. remote_ver, "{FFD700}")
                        M.state.last_notify_ver = remote_ver
                        M.state.last_notify_ver_time = os.time()
                    end
                elseif not silent then
                    notify("\xf3\xf1\xf2\xe0\xed\xee\xe2\xeb\xe5\xed\xe0\x20\xe0\xea\xf2\xf3\xe0\xeb\xfc\xed\xe0\xff\x20\xe2\xe5\xf0\xf1\xe8\xff\x20v" .. SCRIPT_VER, "{00FF88}")
                end
            else
...
function main()
    ...
    if PCS_UPDATE.cfg.autoCheck then
        lua_thread.create(function()
            ...
            local sec = tonumber(PCS_UPDATE_AUTO_SECONDS) or 60
            if sec < 30 then sec = 30 end
            if sec > 600 then sec = 600 end
            while true do
                wait(sec * 1000)

...
PCS_UPDATE = (function()
    ...
    M.cfg.dl_urls = {
        ...
        -- 3) raw.githack
        ('https://raw.githack.com/%s/%s/main/%s'):format(M.cfg.github_user, M.cfg.github_repo, M.cfg.file),
        -- 4) jsDelivr с веткой master (на случай)
        ('https://cdn.jsdelivr.net/gh/%s/%s@master/%s'):format(M.cfg.github_user, M.cfg.github_repo, M.cfg.file),
    }
    ...
    -- временные файлы скачивания (PCStats_http*) — не нужны игроку, всегда чистим
    local function httpTmpPath()
        ...
        return dir .. "/PCStats_http.tmp"
    end

    local function cleanupHttpTmp()
        local dir = "moonloader"
        pcall(function()
            if getWorkingDirectory then
                local w = getWorkingDirectory()
        ...
        -- на всякий случай: любые PCStats_http*
        pcall(function()
            if type(getFiles) == "function" then
                for _, f in ipairs(getFiles(dir .. "/PCStats_http*") or {}) do
                    pcall(os.remove, f)
                end
            end
        end)
    end
    M.cleanupHttpTmp = cleanupHttpTmp

    -- Быстрый HTTP: downloadUrlToFile (как в PCStats 11) + requests + urlmon
    -- version.txt: таймаут 8с; скрипт: 25с. Без зависаний на 60с.
    local function httpGetBody(url, maxWaitMs, minBytes)
        url = tostring(url or "")
        if url == "" then return nil, "empty url" end
        maxWaitMs = tonumber(maxWaitMs) or 12000
        minBytes = tonumber(minBytes) or 1
        cleanupHttpTmp() -- убрать старый PCStats_http*, если остался
        local tryList = { nocache(url) }
        local u, r, b, f = url:match("https://raw%.githubusercontent%.com/([^/]+)/([^/]+)/([^/]+)/(.+)")
        if u then
            -- убрать ?query из имени файла для jsDelivr
            f = tostring(f):match("^([^?]+)") or f
            tryList[#tryList + 1] = nocache(string.format(
                "https://cdn.jsdelivr.net/gh/%s/%s@%s/%s", u, r, b, f))
        end

        for _, tryUrl in ipairs(tryList) do
            -- 1) requests (быстро, если библиотека есть)
            ...
            if okR and requests then
                ...
                if requests.get then
                    ok, response = pcall(requests.get, tryUrl, {
                        headers = { ["User-Agent"] = "PCStats/1.0", ["Cache-Control"] = "no-cache" },
                        timeout = 10,
                    })
                elseif requests.request then
                    ok, response = pcall(requests.request, "GET", tryUrl, {
                        headers = { ["User-Agent"] = "PCStats/1.0" },
                        timeout = 10,
                    })
                end
                if ok and response then
                    ...
                    if code == 200 and type(body) == "string" and #body >= minBytes
            ...
            -- 2) downloadUrlToFile (нативный ML)
            if type(downloadUrlToFile) == "function" then
                local tmp = httpTmpPath()
                pcall(os.remove, tmp)
                local done, knownTotal = false, 0
                pcall(downloadUrlToFile, tryUrl, tmp, function(id, status, p1, p2)
                    status = tonumber(status) or -1
                    p1 = tonumber(p1) or 0
                    p2 = tonumber(p2) or 0
                    if p2 > knownTotal then knownTotal = p2 end
                    -- статусы ML: 1=progress, 2/3/58=done (разные сборки)
                    if status == 2 or status == 3 or status == 58 then
                        done = true
                    elseif status == 4 or status == 6 or status == 7 or status == 9 then
                        done = true
                    end
                end)
                local waited, lastSz, stable = 0, -1, 0
                while waited < maxWaitMs do
                    ...
                    if f then
                        ...
                        local sz = body and #body or 0
                        if sz == lastSz and sz > 0 then stable = stable + 1 else stable = 0; lastSz = sz end
                        local okBody = body and sz >= minBytes
                            and not body:find("404: Not Found", 1, true)
                            and not body:lower():find("<!doctype html", 1, true)
                        if okBody then
                            if knownTotal > 0 and sz >= knownTotal then
                                pcall(os.remove, tmp); return body, nil
                            end
                            if done or stable >= 4 then
                                pcall(os.remove, tmp); return body, nil
                            end
                        end
                    end
                    if done and lastSz < minBytes then break end
                end
                pcall(os.remove, tmp)
            end
    ...
    -- чат: шлём через ОРИГИНАЛЬНЫЙ sampAddChatMessage (без stripUtf8),
    -- чтобы CP1251-кириллица не превращалась в "Г¤Г®Г±..."
    ...
    function M.check(silent)
        ...
        if not silent then
            notify("Checking for updates...", "{66CCFF}")
        end

        lua_thread.create(function()
            local remote_ver = nil
            -- 1) ТОЛЬКО version.txt
            local verUrls = {
                M.cfg.raw_ver,
            ...
            for _, url in ipairs(verUrls) do
                local body = select(1, httpGetBody(url, 8000, 1))
            ...
            -- 2) fallback: SCRIPT_VER from lua on GitHub
            if not remote_ver then
                ...
                for _, url in ipairs(fb) do
                    if url then
                        local body = select(1, httpGetBody(url, 15000, 500))
            ...
            if remote_ver then
                ...
                if M.state.available then
                    ...
                    if needNotify then
                        notify("Update available: v" .. tostring(remote_ver), "{FFD700}")
                    ...
                    if not silent then
                        notify("You have the latest version: v" .. tostring(SCRIPT_VER), "{00FF88}")
                    end
                end
            else
                M.state.last_error = "check failed"
                if not silent then
                    notify("Failed to check version", "{FF6666}")
                end
            end
        end)
    end
    ...
    function M.install()
        if M.state.installing then
            notify("Update already in progress", "{FFAA00}")
            return
        end
        local selfPath = nil
        pcall(function()
        ...
        if not selfPath or selfPath == "" then
            pcall(function()
                if getWorkingDirectory then
                    selfPath = getWorkingDirectory() .. "/PCStats.lua"
                end
            end)
        end
        if not selfPath or selfPath == "" then
            notify("Cannot resolve script path", "{FF6666}")
            return
        end
        ...
        notify("Downloading update...", "{66CCFF}")
        ...
        lua_thread.create(function()
            ...
            local list, seen = {}, {}
            if M.cfg.dl_urls then
                for _, u in ipairs(M.cfg.dl_urls) do
            ...
            local tmpNew = dir .. "/PCStats_update_new.lua"
            local body = nil

            for ui, url in ipairs(list) do
                setP(math.min(5 + (ui - 1) * 8, 40))
                pcall(os.remove, tmpNew)
                local got = false

                if type(downloadUrlToFile) == "function" then
                    local done, fail, knownTotal = false, false, 0
                    local dlUrl = url
                    if dlUrl:find("raw.githubusercontent.com", 1, true) then
                        dlUrl = dlUrl .. ((dlUrl:find("?", 1, true) and "&") or "?") .. "t=" .. tostring(os.time())
                    end
                    local okStart = pcall(downloadUrlToFile, dlUrl, tmpNew, function(id, status, p1, p2)
                        status = tonumber(status) or -1
                        p1 = tonumber(p1) or 0
                        p2 = tonumber(p2) or 0
                        if p2 > knownTotal then knownTotal = p2 end
                        if p2 > 0 then
                            setP(math.min(92, math.floor(5 + p1 / p2 * 85)))
                        elseif p1 > 0 then
                            setP(math.min(90, math.floor(5 + p1 / 600000 * 85)))
                        end
                        if status == 2 or status == 3 or status == 58 then
                            done = true
                        elseif status == 4 or status == 6 or status == 7 or status == 9 then
                    ...
                    if okStart then
                        ...
                        while waited < 45000 do
                            ...
                            local f = io.open(tmpNew, "rb")
                            if f then
                                local sz = f:seek("end") or 0
                                f:close()
                                if sz == lastSz and sz > 0 then stable = stable + 1
                                else stable = 0; lastSz = sz end
                                if knownTotal > 0 and sz >= knownTotal and sz > 20000 then got = true; break end
                                if (done or stable >= 5) and sz > 20000 then got = true; break end
                            end
                            if done and fail then break end
                            if not done and M.state.dlProg < 70 then setP(M.state.dlProg + 0.15) end
                        end
                    end
                end
                ...
                if not got then
                    local okR, requests = pcall(require, "requests")
                    if okR and requests and requests.get then
                        local ok, response = pcall(requests.get, url, {
                            headers = { ["User-Agent"] = "PCStats/1.0" }, timeout = 20,
                        })
                        if ok and response then
                            local code = tonumber(response.status_code or response.status) or 0
                            local txt = response.text or response.content or ""
                            if code == 200 and type(txt) == "string" and #txt > 20000 then
                                local wf = io.open(tmpNew, "wb")
                                if wf then wf:write(txt); wf:close(); got = true end
                            end
                        end
                    end
                end

                if not got and ffi then
                    pcall(function()
                        ffi.cdef[[ int URLDownloadToFileA(void* p, const char* u, const char* f, unsigned long, void*); ]]
                    end)
                    local urlmon
                    ...
                    if urlmon and urlmon.URLDownloadToFileA then
                        pcall(os.remove, tmpNew)
                        if urlmon.URLDownloadToFileA(nil, url, tmpNew, 0, nil) == 0 then
                            local f = io.open(tmpNew, "rb")
                            if f then
                                local sz = f:seek("end") or 0; f:close()
                                if sz > 20000 then got = true end
                            end
                        end
                    end
                end
                ...
                if got then
                    ...
                    if body and isValidBody(body) then
                        setP(93)
                        break
                    end
                    body = nil
            ...
            if not body then
                ...
                setP(0)
                pcall(cleanupHttpTmp)
                pcall(os.remove, tmpNew)
                notify("Failed to download update", "{FF6666}")
                return
            end
            ...
            setP(97)
            local okW = writeAtomic(selfPath, body)
            if not okW then
                pcall(function()
                    local wf = io.open(selfPath, "wb")
                    if wf then wf:write(body); wf:close(); okW = true end
                end)
            end
            if not okW then
                M.state.installing = false
                setP(0)
                pcall(os.remove, tmpNew)
                notify("Failed to write file", "{FF6666}")
                return
            end
            pcall(os.remove, tmpNew)
            pcall(cleanupHttpTmp)
            ...
            notify("Updated to v" .. tostring(newVer or "?") .. ". Reloading...", "{00FF88}")
            wait(800)
            local ok, scr = pcall(thisScript)
            if ok and scr and scr.reload then
                pcall(function() scr:reload() end)
            else
                notify("Reload script manually (F10)", "{FFAA00}")
            end
        end)
    end
...
function main()
    ...
    -- подчистить временные PCStats_http* от прошлых загрузок
    pcall(function() if PCS_UPDATE.cleanupHttpTmp then PCS_UPDATE.cleanupHttpTmp() end end)

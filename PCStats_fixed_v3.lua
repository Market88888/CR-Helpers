...
PCS_UPDATE = (function()
    ...
    -- фоновая проверка версии при старте игры ВЫКЛЮЧЕНА (эффил-потоки + сеть при загрузке);
    -- проверка вручную: /pcsupdate, установка: /pcsupdate install
    M.cfg.autoCheck = false
    ...
    -- effil может отсутствовать в некоторых сборках MoonLoader —
    -- safeRequire() (объявлена в самом начале файла) уже гасит это
    -- через pcall и просто вернёт nil, скрипт при этом не падает
    ...
    function M.check(silent)
        ...
        if not M.effil then
            M.state.last_error = 'no effil'
            if not silent then
                notify("\xce\xe1\xed\xee\xe2\xeb\xe5\xed\xe8\xe5\x20\xed\xe5\xe4\xee\xf1\xf2\xf3\xef\xed\xee\x3a\x20\xed\xe5\xf2\x20\xe1\xe8\xe1\xeb\xe8\xee\xf2\xe5\xea\xe8\x20\x65\x66\x66\x69\x6c", "{FF6666}")
            end
            return
        end
        ...
        lua_thread.create(function()
            local sources = {
                ...
                { url = M.cfg.raw_main,    mode = 'raw'     },
                { url = M.cfg.raw_master,  mode = 'raw'     },
            }
            for _, item in ipairs(sources) do
                local okT, thr = pcall(httpFetch, item.url)
                if not okT then thr = nil end
                if thr then
                    for _ = 1, 180 do
                        wait(100)
                        ...
                        if thread_state == 'completed' then
                            ...
                            if ok and response and tonumber(response.status_code) == 200 and response.text ~= '' then
                                ...
                                elseif item.mode == 'tags' then
                                    ...
                                    remote_ver = verFromBody(response.text)
                                end
                                if remote_ver then
                                    ...
                                    if M.state.available then
                                        notify("\xe4\xee\xf1\xf2\xf3\xef\xed\xee\x20\xee\xe1\xed\xee\xe2\xeb\xe5\xed\xe8\xe5\x20v" .. remote_ver ..
    ...
    function M.install()
        ...
        lua_thread.create(function()
            local sources = {
                { url = M.cfg.release_dl },
                { url = M.cfg.raw_main   },
                { url = M.cfg.raw_master },
            }
            for _, item in ipairs(sources) do
                local okT, thr = pcall(httpFetch, item.url)
                if not okT then thr = nil end
                    ...
                    for _ = 1, 260 do
                        ...
                        if thread_state == 'completed' then
                            ...
                            if ok2 and response and tonumber(response.status_code) == 200 then
                                ...
                                if isValidBody(body) then
                                    local newVerChk = verFromBody(body)
                                    local compiled, compErr = loadstring(body)
                                    if not compiled then
                                        M.state.installing = false
                                        M.state.last_error = 'remote does not compile'
                                        print('[PC Stats][update] remote file rejected, does not compile: ' .. tostring(compErr))
                                        notify("", "{FF6666}")
                                        return
                                    end
                                    if not newVerChk or not verLt(SCRIPT_VER, newVerChk) then
                                        M.state.installing = false
                                        M.state.available = false
                                        print('[PC Stats][update] install refused: remote version ' .. tostring(newVerChk) .. ' is not newer than ' .. SCRIPT_VER)
                                        notify("\xd3\xf1\xf2\xe0\xed\xee\xe2\xea\xe0\x20\xee\xf2\xec\xe5\xed\xe5\xed\xe0\x3a\x20\xed\xe0\x20\x47\x69\x74\x48\x75\x62\x20\x76" .. tostring(newVerChk or '?') .. "\x2c\x20\xeb\xee\xea\xe0\xeb\xfc\xed\xe0\xff\x20\x76" .. SCRIPT_VER .. "\x20\x97\x20\xed\xee\xe2\xe5\xe5\x20\xed\xe5\xf2", "{FFAA00}")
                                        return
                                    end
                                    pcall(function()
                                        local rf = io.open(self_path, 'rb')
                                        if rf then
                                            local dataOld = rf:read('*a'); rf:close()
                                            local bf = io.open(self_path .. '.bak', 'wb')
                                            if bf then bf:write(dataOld); bf:close() end
                                        end
                                    end)
                                    if not writeAtomic(self_path, body) then
                                        M.state.installing = false
                                        M.state.last_error = 'write failed'
                                        notify("\xce\xf8\xe8\xe1\xea\xe0\x20\xe7\xe0\xef\xe8\xf1\xe8\x20\xf4\xe0\xe9\xeb\xe0", "{FF6666}")
                                        return
                                    end
                                    local new_ver = verFromBody(body) or M.state.ver_remote or '?'
                                    notify("\xee\xe1\xed\xee\xe2\xeb\xb8\xed\x20\xe4\xee\x20v" .. new_ver .. "\x2e\x20\xef\xe5\xf0\xe5\xe7\xe0\xef\xf3\xf1\xea\xe0\xfe\x2e\x2e\x2e", "{00FF88}")
                                    wait(300)
            ...
            notify("\xcd\xe5\x20\xf3\xe4\xe0\xeb\xee\xf1\xfc\x20\xf1\xea\xe0\xf7\xe0\xf2\xfc\x20\xee\xe1\xed\xee\xe2\xeb\xe5\xed\xe8\xe5\x20\xf1\x20\x47\x69\x74\x48\x75\x62", "{FF6666}")
        end)
    end
...
imgui.OnInitialize(function()
    ...
    pcall(function()
        ...
        -- ФИКС: если в атласе ещё нет ни одного шрифта, ConfigData.Data == NULL,
        -- и чтение Data[0] — разыменование нулевого указателя (нативный вылет,
        -- pcall его не ловит). Сначала убеждаемся, что базовый шрифт есть.
        if fonts.ConfigData.Size <= 0 then fonts:AddFontDefault() end
        local baseSize = 13.0
        if fonts.ConfigData.Size > 0 then
            local defaultFontCfg = fonts.ConfigData.Data[0]
            baseSize = (defaultFontCfg and defaultFontCfg.SizePixels) or 13.0
        end
        local config = imgui.ImFontConfig()
        config.MergeMode   = true
        config.PixelSnapH  = true
        -- ФИКС: диапазон глифов ImGui хранит как УКАЗАТЕЛЬ и читает при сборке
        -- атласа; локальная cdata-переменная могла быть собрана GC раньше.
        -- Глобальная ссылка держит массив живым всё время работы скрипта.
...
function drawAboutInner(h)
    if St._resetSettScroll then imgui.SetScrollY(0) end
        ...
        aboutCard("##updcard", 96, function(aw, ch)
            imgui.SetWindowFontScale(aboutBaseScale)
            local U = PCS_UPDATE
    ...
    function toggleMenuWindow()
        ...
        do
            ...
            if not PCS_TRACE_SHOWN then
                ...
                print("[PC Stats] trace file: " .. PCS_TRACE_PATH)
                pcall(sampAddChatMessage, "{AAAAAA}[PC Stats] trace: " .. PCS_TRACE_PATH, -1)
            end
        end
